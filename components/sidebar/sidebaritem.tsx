...
const SideBarItem: React.FC<SideBarItemProps> = ({ href = '', isMenu, children, startContent, endContent, menuContent }) => {
    ...
    const handleMenuClick = () => {
        const height = (menuContent?.length + 1) * 28
                ...
                <>
                    ...
                    <div className='flex ml-8 flex-col flex-nowrap gap-1 max-h-0 overflow-hidden transition-menu ' ref={menuRef}>
                        {menuContent?.map(item => (
                            <Link key={item.name} className={`py-1 px-4 text-sm rounded-lg ${pathname === item.href && item.href != '' && 'bg-match-link'} `} href={item.href}>{item.name}</Link>
                        ))}
                    </div>
                </>
                :
                <Link className={`flex flex-row flex-nowrap justify-between items-center w-full gap-2 py-2 px-4 text-gray-700  transition-colors rounded-lg     ${pathname === href && href != '' && 'bg-match-link'} transition-colors duration-500`} href={href}>
                    <div className='flex justify-start items-center gap-4'>
                        {startContent}{children}
                    </div>
                    {endContent}
